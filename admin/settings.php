...
// Handle display name update for admin
...
$profile = ['full_name' => $_SESSION['user_name'] ?? 'Admin', 'email' => $_SESSION['user_email'] ?? ''];
?>
<!doctype html>
<html lang="en">
  <head>
    ...
    <div class="max-w-6xl mx-auto px-4 py-6 grid grid-cols-1 md:grid-cols-12 gap-6">
      ...
      <main class="md:col-span-9 space-y-6">
        ...
        <div class="bg-white rounded-lg shadow-sm p-4">
          <h1 class="text-lg font-semibold text-slate-800">Profile</h1>
          <form action="settings.php" method="post" class="mt-4 space-y-4 max-w-lg">
            <div>
              <label class="block text-sm font-medium text-slate-700">Full Name</label>
              <input name="full_name" type="text" required class="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" value="<?= htmlspecialchars($profile['full_name']) ?>">
            </div>
            <div>
              <label class="block text-sm font-medium text-slate-700">Email</label>
              <input type="email" disabled class="mt-1 block w-full rounded-md border-slate-300 bg-slate-50" value="<?= htmlspecialchars($profile['email']) ?>">
            </div>
            <button type="submit" class="inline-flex justify-center rounded-md bg-slate-800 px-4 py-2 text-white text-sm font-medium hover:bg-slate-900">Save</button>
          </form>
        </div>
      </main>
    </div>
